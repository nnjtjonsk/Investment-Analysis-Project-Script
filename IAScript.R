# Investment Analysis Assignment - Questions 1, 2, & 3
# FINAL, COMPLETE, AND PORTABLE SCRIPT
# This script performs all required analyses and is designed to be shared.

# --- 0. Setup: Load Libraries ---
# If you run this on a new machine, you may need to install these packages first.
...
# The line below uses a RELATIVE PATH.
# This requires the "InvestmentMastersheet.xlsx" file to be in the SAME FOLDER as this R script.
...
# CORRECTED LOGIC: Consolidate data by PERMNO before analysis
...
# CORRECTED LOGIC: Consolidate data by PERMNO before analysis for the 10-year sample
...
# This ensures one consolidated beta per company, regardless of ticker changes.
beta_estimates_raw <- regression_data %>%
  group_by(PERMNO) %>% # MODIFIED: Group only by the unique company identifier
  filter(n() > 24) %>% # Ensure at least 2 years of data
...
# It's crucial to group by PERMNO to get one average return for each unique company.
...
# We use an inner_join to ensure we only plot companies for which we have both a beta and an average return.
...
return_beta_scatterplot <- ggplot(plot_data, aes(x = beta, y = Avg_Excess_Return)) +
  ...
  # Add a simple line of best fit to help visualize the relationship
  ...
  # Format axes to be more readable (as percentages)
...
cat("--- Question 5 finished successfully. ---\n")
  ...
  # This is the Fama-MacBeth second-stage regression
  ...
  sml_plot <- ggplot(sml_data, aes(x = beta, y = Avg_Excess_Return)) +
    geom_point(alpha = 0.6, color = "blue") +
    # Add the formally estimated SML using the coefficients from our model
    geom_abline(
  ...
  # CORRECTED: This block de-duplicates the book value data before merging to prevent the warning.
  book_value_clean <- book_value_df %>%
    rename(PERMNO = LPERMNO, BV_date = datadate, BVPS = bkvlps) %>%
    mutate(fyear = year(BV_date)) %>%
    # ADDED: De-duplicate by taking the first observation for each company-year
